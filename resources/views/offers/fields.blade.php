    ...
    <div class="row">
        ...
        <div class="col-md-3">
            <ul class="nav nav-pills nav-stacked admin-menu" >
                ...
                <li><a href="" data-target-id="Arabic"><i class="glyphicon glyphicon-edit"></i> الرابط والخصم  </a></li>
        ...
        <div class="col-md-8  admin-content" id="profile">
            ...
            <div class="panel panel-primary" style="margin: 1em;">
                ...
                <div class="panel-body">
                    <select name="store_id" id="city" class="js-states form-control">
                        @foreach($stores as $store)   
                        @if(request('lang'))
                        <option {{isset($offer) && $offer->store_id==$store->storeLangs->where('lang_id','=',request('lang'))->first()->store_id?"selected":" "}}
                        value='{{$store->id}}'> {{$store->storeLangs->where('lang_id','=',request('lang'))->first()->name}} </option>
    
                        @else
                        <option {{isset($offer) && $offer->store_id==$store->storeLangs->where('lang_id','=','2')->first()->store_id?"selected":" "}}
                            value='{{$store->id}}'> {{$store->storeLangs->where('lang_id','=','2')->first()->name}} </option>
